...
**Paused 2026-08-26, same system.** Question paper/subjective paper authoring, the Controls screen, and the Database/Theme admin tabs are committed, pushed, and deployed to production. A follow-up round of report-card watermark fixes (transparency, position, sidebar borders) found while testing Theme is done locally — commit/push/deploy that before starting the next feature. **Lesson planner + its AI-generation flow** is next up in "What's deferred" below (needs a Gemini API key — get one before starting).
...
- **Controls** (Admin only): a generic toggle-switch settings screen for the `controls` feature-flag table (login gates, marks-feeding locks, term/report selectors, etc.), grouped by `ctype` with the same conid/name overrides as the source (marks-feeding rows forced into one group regardless of their own `ctype`; "Password Reset" pulled into a Security group). This tab existed in the nav (and its backing `lib/controls.php` helpers) since early in the project but had no UI until now — `ctype='theme'` rows (Default Theme, Report Watermark) are deliberately excluded — they're managed on the dedicated Theme tab instead (see below), and the report-card export endpoints already read the `Report Watermark` row's `cdata`/`cval` directly (`api/report-card/export_excel.php`, `api/final-results/export_excel.php`), so this screen isn't the only writer that needs to stay in sync with it.
- **Responsive/collapsible main navigation**: `dashboard.php`'s tab bar (up to 16 tabs for Admin) is a proper Bootstrap `navbar-expand-lg`, collapsing behind a hamburger button below the `lg` breakpoint with full-width stacked tab rows, auto-closing on tab selection (`dashboard.js`). All 16 tables across the app already sit in `.table-responsive` wrappers so they scroll rather than break layout on narrow screens; verified structurally (standard Bootstrap markup) but never visually checked on a real phone/tablet — no browser was available in the session this was built.
- **Theme** (Admin only): a real, working port of `theme-management.tsx` — this was previously (incorrectly) written off in this README as source-only stub code. Site-wide Light/Dark toggle via Bootstrap 5.3's native `data-bs-theme`, set server-side on `<html>` on every entry page (`login.php`, `dashboard.php`, `student_dashboard.php`) from the `Default Theme` control row to avoid a flash of the wrong theme. Bootstrap's own components re-theme automatically; this app's few hardcoded-color classes (`app-header`, `app-shell`, etc.) get explicit dark overrides in `app.css` — third-party widget chrome (DataTables, SweetAlert2) is not re-themed. Also a report-card watermark uploader (image + size slider, opacity baked into the composited PNG client-side via canvas before upload, matching the source), persisted into the `Report Watermark` control row that `api/report-card/export_excel.php` and `api/final-results/export_excel.php` already read on every export — no separate wiring needed, those endpoints predate this tab and already sourced the watermark from this same control. Fixed a real bug found while verifying this: `lib/report_card_excel.php` decoded the uploaded PNG via bare `imagecreatefromstring()` without the follow-up `imagesavealpha()` call PhpSpreadsheet's own `MemoryDrawing::fromString()` factory does internally — GD silently drops the alpha channel once the resource gets cloned per student sheet, so every watermark rendered fully opaque regardless of the transparency baked into the upload. Confirmed root cause and fix by extracting the embedded image from a generated `.xlsx` before/after and checking its actual pixel alpha value. Also moved the watermark's anchor from `G6` to `F3` and gave the report card's LEGEND and GRADE SUBJECTS sidebar blocks a real outside border — the LEGEND one was a genuine bug (border style was applied to only the merged range's top-left cell reference instead of the full range, so just the top-left corner rendered); GRADE SUBJECTS' fragile per-row manual left/right/bottom-on-last-row styling was replaced with a single call to PhpSpreadsheet's `getOutline()` range border, which draws a clean box around a range's true outer edge regardless of the merges inside it. Verified via the real generator function's output cell styles, not just visual inspection.
- **Database** (Admin only): also previously written off as a stub — it's real. A full SQL backup/download (streamed table-by-table rather than the source's build-one-giant-string-in-memory approach, since this database is large; restore-tested locally by re-importing the generated dump into a throwaway database and confirming matching table/row counts) and a live database-connection editor. The connection editor adds real safety the source's own `setDbConfig` lacks entirely: it test-connects with the new credentials (and confirms a `teachers` table exists, as a proxy for "this looks like the right database") *before* writing anything, and backs up `config.php` (timestamped copy, git-ignored) before overwriting it — the source blindly rewrites its `.env` with zero verification. **This is still a genuinely dangerous feature**: config.php is required by every page in the app, and a mistake here (or the underlying database becoming unreachable for unrelated reasons right after a save) takes the whole site down until someone with SFTP access restores the backup. Deliberately **not** ported: the source's "Reset Database" tool, whose hardcoded 18-table truncate list is stale against this app's actual schema (missing `tcissued`, `data_collection_*`, timetable tables, `aptitude_marks`, and others) and has a real data-integrity bug — truncating `students` resets its `AUTO_INCREMENT`, so a student created after a reset can inherit an old, deleted student's leftover records in the tables that don't get cleared.

## What's deferred

Not built yet, in roughly ascending order of effort/risk:

- **DB reset tool** — deliberately not ported; the source's implementation has a real data-integrity bug (see "Database" above under What's implemented). Would need a from-scratch table list generated against this app's actual current schema, not a straight port.
- SMS/WhatsApp (stub-only in the source — nothing functional to port there; unlike the database/theme tabs, this one really is just dead/unwired code)
...
8. `communications` — had a single inline `sclass NOT NULL` column with no `communication_recipients` join table at all; the source's current code assumes the multi-class-recipients design and would fail on any insert. The table had zero rows, so `communication_recipients` was created fresh and `communications.sclass` relaxed (nullable, unused going forward) rather than migrating anything. **Not in `vsecadlu_reportcard202627.sql`** (that dump predates this fix) — a local DB rebuilt from it will hit `Table 'communication_recipients' doesn't exist` on the Communications tab until this fix is reapplied locally (see the `CREATE TABLE`/`ALTER TABLE` above; production already has it, this is local-only).
