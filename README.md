...
- **Controls** (Admin only): a generic toggle-switch settings screen for the `controls` feature-flag table (login gates, marks-feeding locks, term/report selectors, etc.), grouped by `ctype` with the same conid/name overrides as the source (marks-feeding rows forced into one group regardless of their own `ctype`; "Password Reset" pulled into a Security group). This tab existed in the nav (and its backing `lib/controls.php` helpers) since early in the project but had no UI until now — `ctype='theme'` rows (Default Theme, Report Watermark) are deliberately excluded, since this port's report-card export already handles watermarking as a per-export upload rather than reading it from this table.
